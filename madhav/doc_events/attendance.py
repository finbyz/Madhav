...
def set_status(self, method):
    if not self.in_time and not self.out_time: 
        from madhav.api import get_offday_status     
        if not self.in_time and not self.out_time:        
...
def set_short_leave_count(self, method):
    ...
    original_short_leave_count = self.short_leave_count if hasattr(self, 'short_leave_count') else None
    
    if is_first_attendance_of_month(self):
        self.db_set("short_leave_count",frappe.db.get_single_value("Attendance Settings", 'short_leave_in_month'))
        ...
        previous_attendance = frappe.get_list("Attendance", 
                               ...
                               fields=['short_leave_count','name'], 
                               order_by='modified desc', 
                               limit_page_length=1)
        
        if previous_attendance and len(previous_attendance) > 0:
            self.db_set("short_leave_count", previous_attendance[0].get('short_leave_count'))
    ...
    if self.short_leave_count <= 0:
        return
    
    # Store the current short leave count after initial assignment
    current_short_leave_count = self.short_leave_count
    ...
    for row in short_leave_list:
        ...
        if (in_time_diff and start <= in_time_diff <= end) or (out_time_diff and start <= out_time_diff <= end):
            ...
            before_deduction = self.short_leave_count
            self.short_leave_count = self.short_leave_count - deduction
            deduction_applied = True
            
            # Update the status directly in the database if after submission
            if is_after_submit and  self.short_leave_count > 0 :
                
                self.db_set('status','Present With Short Leave')
                
                self.db_set('short_leave_count',self.short_leave_count)
                ...
                #         data_to_recreate["short_leave_count"] = self.short_leave_count  # explicitly preserve it
